...
from src.log_utils.read_log import *

filepath = input("filepath (default = python_test/data/input/qgames.log): ")
if not filepath:
    filepath = '../data/input/qgames.log'

file_log = import_data(filepath)
joined_info_to_divide = prepare_to_divide_by_match(file_log)
init_lines = joined_info_to_divide[0]
total_matches = joined_info_to_divide[1]
log_by_match = divide_log_by_match(file_log, init_lines, total_matches)
ranking = create_ranking_dict(log_by_match, total_matches)
add_score_to_ranking(ranking, log_by_match, total_matches)
ranking_by_death = create_ranking_by_death_dict(log_by_match, total_matches)
